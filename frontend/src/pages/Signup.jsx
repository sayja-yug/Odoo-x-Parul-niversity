...
export default function Signup() {
  ...
  return (
    <div className="grid min-h-screen lg:grid-cols-2">
      <section className="hidden bg-[radial-gradient(circle_at_top_right,_rgba(245,158,11,.18),_transparent_32%),linear-gradient(180deg,#07111f_0%,#0b1730_100%)] p-8 lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="max-w-xl">
          <p className="text-sm uppercase tracking-[0.2em] text-aqua-200">Collaborative planning</p>
          <h1 className="mt-4 text-5xl font-semibold tracking-tight text-white">Turn trip ideas into a polished plan with maps, budgets, and notes.</h1>
        </div>
      </section>

      <section className="flex items-center justify-center p-6 sm:p-10">
        <motion.form initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleSubmit} className="w-full max-w-md rounded-[2rem] border border-white/10 bg-white/5 p-8 shadow-soft backdrop-blur-xl">
          ...
          <div className="mt-8 space-y-4">
            ...
            <label className="block">
              ...
              <div className="relative">
                ...
                <input type="email" className="w-full rounded-2xl border-white/10 bg-white/5 py-3 pl-11 pr-4 text-white placeholder:text-slate-500 focus:border-aqua-400 focus:ring-aqua-400" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} placeholder="you@example.com" />
              </div>
            </label>
            <label className="block">
          ...
          <p className="mt-5 text-center text-sm text-slate-400">
            Already have an account? <Link to="/login" className="text-aqua-200 hover:text-white">Sign in</Link>
          </p>
        </motion.form>
      </section>
    </div>
